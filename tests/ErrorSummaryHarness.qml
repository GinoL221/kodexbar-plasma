...
Item {
    ...
    Component.onCompleted: {
        ...
        assert(summary.disclosureButton.Accessible.name.indexOf("23") !== -1, "the disclosure must expose its count to assistive technology")
        summary.expanded = true
        assert(summary.expanded, "the disclosure state must be keyboard-toggleable")
        summary.disclosureButton.forceActiveFocus()
        assert(summary.disclosureButton.activeFocus, "the disclosure must retain focus for Return and Space activation")
